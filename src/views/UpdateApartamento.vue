...
export default {
  ...
  methods: {
    refreshApartamento() {
      ApartamentoService.retrieveApartamento(this.INSTRUCTOR, this.id).then((res) => {
        if (this.id != -1) {
    ...
    validateAndSubmit(e) {
      ...
      if (this.errors.length === 0) {
        if (this.id === -1) {
          ...
          ApartamentoService.updateApartamento(this.INSTRUCTOR, this.id, {
            ...
            this.$router.push("/apartamento");
            console.log(
              `update apartamento --> SIGLA: ${this.sigla}, NOME: ${this.nome}`
            );
          });
        }
      }
      window.Toast.fire(
        "Success",
        `Apartamento "${this.nome}" was registred/updated!`,
        "success"
      );
    },
  },
